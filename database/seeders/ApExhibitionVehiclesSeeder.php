...
class ApExhibitionVehiclesSeeder extends Seeder
{
  ...
  public function run(): void
  {
    ...
    $color = ApMasters::first();
    $engineType = ApMasters::skip(1)->first();
    $typeOperation = ApMasters::skip(2)->first();

    if (!$supplier || !$warehouse || !$advisor || !$vehicleStatus || !$model || !$color || !$engineType || !$typeOperation) {
      $this->command->error('Faltan datos maestros. Asegúrate de tener registros en: business_partners, warehouse, workers, ap_vehicle_status, ap_models_vn, ap_masters');
      return;
    }
