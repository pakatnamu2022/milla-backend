...
class SyncSalesDocumentJob implements ShouldQueue
{
  ...
  protected function processElectronicDocument(int $electronicDocumentId, DatabaseSyncService $syncService): void
  {
    $document = ElectronicDocument::with([
      ...
      'orderQuotation.deductibles.electronicDocument',
  ...
  private function syncDocFVStep(ElectronicDocument $document): void
  {
    ...
    try {
      ...
      if (!$existing) {
        ...
        $area = $areaMapping[$document->area_id] ?? 'POSTVENTA';
        ...
        DB::connection(Company::CONNECTION_DYNAMICS_3)
          ...
          ->insert([
            ...
            'Modulo' => in_array($document->area_id, [ApMasters::AREA_POSVENTA, ApMasters::AREA_TALLER, ApMasters::AREA_MESON]) ? 'POSTVENTA' : 'COMERCIAL',
            'Area' => $area,
          ]);
      }
