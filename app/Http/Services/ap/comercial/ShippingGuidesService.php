...
class ShippingGuidesService extends BaseService implements BaseServiceInterface
{
  ...
  public function list(Request $request)
  {
    ...
    $query = ShippingGuides::where(function ($q) use ($sedes) {
      $q->whereIn('sede_transmitter_id', $sedes)->orWhereIn('sede_receiver_id', $sedes);
    });
  ...
  public function syncWithDynamics($id)
  {
    ...
    if ($shippingGuide->is_accounted && $shippingGuide->status) {
      $isEligibleComercialTransfer = $shippingGuide->area_id === ApMasters::AREA_COMERCIAL
        && $shippingGuide->transfer_reason_id !== null
