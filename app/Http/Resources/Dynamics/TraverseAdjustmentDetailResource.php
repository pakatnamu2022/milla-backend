...
class TraverseAdjustmentDetailResource extends JsonResource
{
  ...
  public function toArray(Request $request): array
  {
    ...
    // PASO 1: Agrupar por fecha de compra -> producto
    ...
    foreach ($traverseItems as $item) {
      ...
      foreach ($item->linkTransactions as $transaction) {
        ...
        // Crear clave única por fecha + producto
        $key = $purchaseDate . '_' . $product->id;

        if (!isset($groupedByPurchaseDate[$purchaseDate])) {
          $groupedByPurchaseDate[$purchaseDate] = [];
        }

        if (!isset($groupedByPurchaseDate[$purchaseDate][$key])) {
          $groupedByPurchaseDate[$purchaseDate][$key] = [
            'product' => $product,
            'warehouse' => $warehouse,
        ...
        $groupedByPurchaseDate[$purchaseDate][$key]['total_quantity'] += $quantity;
        $groupedByPurchaseDate[$purchaseDate][$key]['total_cost'] += $quantity * $unitCost;
      }
    }

    // PASO 2: Generar las líneas del ajuste para cada fecha de compra
    foreach ($groupedByPurchaseDate as $purchaseDate => $products) {
      $lineNumber = 1;
      ...
      foreach ($products as $key => $data) {
        ...
        $allDetails[] = [
          ...
          'Motivo' => $this->isReversal ? 'Reversión travesía' : 'Ajuste travesía',
